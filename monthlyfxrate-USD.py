...
newfxrate = fxrate[fxrate.Currency.isin(["AUD","CAD","CHF","EUR","GBP","INR","MXN","PHP","PEN","PLN","CNY","SGD","NZD","HKD"])]
finalfx = newfxrate[['Currency', 'Units per USD', 'USD per unit', 'Date']]

oldfx = pd.DataFrame(pd.read_csv('monthlyfxrate-USD.csv')).round({"Units per USD":3, "USD per unit":3})
